...
import { Sparkles, Star, Zap, ShieldCheck, ArrowRight } from 'lucide-react';
...
const NewArrivals = ({ onAddToCart }) => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/products`)
      .then(res => res.json())
      .then(data => {
        // Filter new arrival items (tagged as new, recently created, or labeled New Arrival)
        ...
        // Fallback to top 4 products if tag filtering yields fewer items
  ...
  return (
    <div className="new-arrivals-page" style={{ background: '#f8fafc', paddingBottom: '60px' }}>
      {/* Emerald Hero Banner */}
      <div className="new-arrivals-banner" style={{ background: 'linear-gradient(135deg, #064e3b 0%, #047857 50%, #10b981 100%)', color: '#ffffff', padding: '50px 0', borderBottom: '3px solid #34d399', boxShadow: '0 4px 20px rgba(0,0,0,0.15)' }}>
        <div className="container new-arrivals-banner-inner" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div className="new-arrivals-banner-text">
            <span style={{ background: 'rgba(255,255,255,0.2)', color: '#a7f3d0', fontSize: '12px', fontWeight: '700', padding: '4px 14px', borderRadius: '20px', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '12px', display: 'inline-block' }}>
              JUST RELEASED THIS WEEK
            </span>
            <h2 style={{ fontSize: '36px', fontWeight: '800', margin: '8px 0', textShadow: '0 2px 4px rgba(0,0,0,0.2)' }}>
              ✨ Fresh New Arrivals
            </h2>
            <p style={{ fontSize: '16px', color: '#ecfdf5', maxWidth: '600px', lineHeight: '1.5' }}>
              Be the first to explore groundbreaking gadgets, new fashion drops, and premium innovations freshly added to GlobalJD.
            </p>
          </div>
          <div className="new-arrivals-badge" style={{ background: 'rgba(255,255,255,0.15)', padding: '20px', borderRadius: '50%', border: '2px solid rgba(167,243,208,0.4)', backdropFilter: 'blur(10px)' }}>
            <Sparkles size={48} color="#a7f3d0" />
          </div>
        </div>
      </div>
      ...
      <section className="container" style={{ marginTop: '36px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'center', marginBottom: '36px' }}>
          {[
            ...
            { key: 'electronics', label: 'Latest Electronics' },
            { key: 'fashion', label: 'New Fashion Drops' },
            ...
            <button
              ...
              style={{
                padding: '10px 22px', borderRadius: '25px', border: activeCategory === cat.key ? '2px solid #059669' : '1px solid #cbd5e1',
                background: activeCategory === cat.key ? '#059669' : '#ffffff', color: activeCategory === cat.key ? '#ffffff' : '#334155',
                fontSize: '14px', fontWeight: '600', cursor: 'pointer', boxShadow: activeCategory === cat.key ? '0 4px 12px rgba(5,150,105,0.25)' : '0 1px 3px rgba(0,0,0,0.05)',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <h2 className="section-title" style={{ margin: 0, fontSize: '24px', fontWeight: '800', color: '#0f172a' }}>
            Latest Products Added ({filteredProducts.length})
          </h2>
          <span style={{ fontSize: '13px', color: '#059669', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '4px' }}>
            <Zap size={15} /> 100% Brand New Release
          </span>
        </div>
        ...
        {filteredProducts.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', background: '#ffffff', borderRadius: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
            <h3 style={{ fontSize: '18px', color: '#334155', marginBottom: '8px' }}>No new arrivals found in this filter</h3>
            ...
            {filteredProducts.map(product => (
              ...
              >
                <div className="product-image" style={{ height: '260px', background: '#f8fafc', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ position: 'absolute', top: '12px', left: '12px', background: '#059669', color: '#fff', padding: '4px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: '700', zIndex: 2 }}>
                    NEW ARRIVAL
                  </span>
                  {product.brand && (
                ...
                <div className="product-info" style={{ padding: '20px', display: 'flex', flexDirection: 'column', flex: 1 }}>
                  {/* Category Pill */}
                  <span style={{ fontSize: '11px', color: '#059669', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '6px' }}>
                    {product.category}
                  </span>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px', fontSize: '12px', color: '#64748b' }}>
                    <span style={{ color: '#d97706', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '3px' }}>
                      <Star size={13} fill="#d97706" /> {product.ratingValue || (product.rating ? product.rating.split(' ')[0] : '4.9')}
                    </span>
                    <span>•</span>
                    <span style={{ fontSize: '11px', color: '#059669', fontWeight: '600' }}>{product.stockStatus || 'In Stock'}</span>
                  </div>
