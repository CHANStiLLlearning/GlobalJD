...
const Fashion = ({ onAddToCart }) => {
    ...
    return (
      <section className="container" style={{ minHeight: '60vh', padding: '60px 0', textAlign: 'center' }}>
        <div style={{ fontSize: '18px', color: '#64748b', fontWeight: '500' }}>
          ...
          Loading Luxury Fashion & Apparel Collection...
        </div>
      </section>
    );
  }
  ...
  return (
    <div className="fashion-page" style={{ background: '#f8fafc', paddingBottom: '60px' }}>
      {/* Premium Hero Banner */}
      <div className="fashion-banner" style={{ background: 'linear-gradient(135deg, #1e1b4b 0%, #312e81 50%, #4338ca 100%)', color: '#ffffff', padding: '50px 0', borderBottom: '3px solid #818cf8', boxShadow: '0 4px 20px rgba(0,0,0,0.15)' }}>
        <div className="container fashion-banner-inner" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div className="fashion-banner-text">
            <span style={{ background: 'rgba(255,255,255,0.15)', color: '#a5b4fc', fontSize: '12px', fontWeight: '700', padding: '4px 14px', borderRadius: '20px', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '12px', display: 'inline-block' }}>
            ...
            <p style={{ fontSize: '16px', color: '#e0e7ff', maxWidth: '600px', lineHeight: '1.5' }}>
              Discover designer apparel, handcrafted leather goods, luxury eyewear, and signature shoes with full specification details.
            </p>
          </div>
          <div className="fashion-icon" style={{ background: 'rgba(255,255,255,0.1)', padding: '20px', borderRadius: '50%', border: '2px solid rgba(255,255,255,0.2)', backdropFilter: 'blur(10px)' }}>
            <ShoppingBag size={48} color="#a5b4fc" />
          </div>
        </div>
      </div>
      ...
      <section className="container" style={{ marginTop: '36px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'center', marginBottom: '36px' }}>
          {[
            ...
            <button
              ...
              style={{
                padding: '10px 22px', borderRadius: '25px', border: activeCategory === cat.key ? '2px solid var(--jd-red)' : '1px solid #cbd5e1',
                background: activeCategory === cat.key ? 'var(--jd-red)' : '#ffffff', color: activeCategory === cat.key ? '#ffffff' : '#334155',
                fontSize: '14px', fontWeight: '600', cursor: 'pointer', boxShadow: activeCategory === cat.key ? '0 4px 12px rgba(226,35,26,0.25)' : '0 1px 3px rgba(0,0,0,0.05)',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <h2 className="section-title" style={{ margin: 0, fontSize: '24px', fontWeight: '800', color: '#0f172a' }}>
            Trending Apparel & Accessories ({filteredProducts.length})
          </h2>
          <span style={{ fontSize: '13px', color: '#64748b', fontWeight: '500' }}>
            Showing 100% Authentic Global Fashion Items
          </span>
        </div>
        ...
        {filteredProducts.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', background: '#ffffff', borderRadius: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
            <h3 style={{ fontSize: '18px', color: '#334155', marginBottom: '8px' }}>No items found in this category</h3>
          ...
          <div className="product-grid">
            {filteredProducts.map(product => (
              ...
              >
                <div className="product-image" style={{ height: '280px', background: '#f8fafc', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {product.discount && (
                  ...
                  <img src={product.image} alt={product.name} style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain', transition: 'transform 0.3s ease' }} />
                </div>
                  ...
                  {product.sizes && product.sizes.length > 0 && (
                    <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginBottom: '12px' }}>
                      {product.sizes.slice(0, 4).map((sz, i) => (
                        <span key={i} style={{ fontSize: '10px', background: '#f1f5f9', color: '#475569', padding: '2px 6px', borderRadius: '4px', fontWeight: '600' }}>
                          {sz}
                        </span>
                      ))}
                    </div>
